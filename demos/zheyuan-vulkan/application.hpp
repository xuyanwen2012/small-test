...
VkPipelineShaderStageCreateInfo load_shader(const std::string shader_name, VkShaderModule *shaderModule){
	const std::string shadersPath = "/data/local/tmp/";
...
void create_shared_empty_storage_buffer(const VkDeviceSize bufferSize, VkBuffer* device_buffer, VkDeviceMemory* device_memory, void** mapped) {
    // Add memory allocation flags for Android compatibility
    ...
    // Add HOST_CACHED flag for Android
    VkMemoryPropertyFlags memProps = 
        VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT |
        VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
        VK_MEMORY_PROPERTY_HOST_COHERENT_BIT |
        VK_MEMORY_PROPERTY_HOST_CACHED_BIT;  // Add this flag
    ...
    // Add error handling for memory mapping
    VkResult result = vkMapMemory(singleton.device, *device_memory, 0, bufferSize, 0, mapped);
    if (result != VK_SUCCESS) {
        std::cout << "Memory map failed with error code: " << result << std::endl;
    }
}
