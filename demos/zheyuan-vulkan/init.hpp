...
void Init::run(const int blocks, const int queue_idx, glm::vec4* data, VkBuffer data_buffer, const int n, const int min_val, const float range, const float seed){
	...
	// for (const auto& entry : std::filesystem::directory_iterator("/data/local/tmp/")) {
	// 	std::cout << entry.path().filename() << std::endl;
	// }


	// for all shader, look for them in the ""/data/local/tmp/"" directory
	// prepend the directory to the shader name
